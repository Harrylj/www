$(function($) {
	...
	*/
	...
	$(settings.element).on('click',function(){
	  var click_class = 'this_hover';
	  if($(this).hasClass(click_class)){
	    $(this).stop().animate({opacity: settings.start_opacity}, settings.timing); //return hovered object to start opacity
      $(this).siblings().removeClass(click_class).stop().animate({opacity: settings.start_opacity}, settings.timing); // return other objects to start opacity
	    $(this).removeClass(click_class)
	  }
	  else{
	    $(this).stop().animate({opacity: settings.high_opacity}, settings.timing); //100% opacity for hovered object
      $(this).siblings().removeClass(click_class).stop().animate({opacity: settings.low_opacity}, settings.timing); //dimmed opacity for other objects
