            ...
            a.app = {
                ...
                buildResult: function(c, d, e) {
                    ...
                    var i = a.app.getOpenUrl(d),
                        j = this;
                    return h && !d.openUrl && (1 == g && (i += "&callback=weixin://&sender=å¾®ä¿¡"), 2 == g && (i += "&callback=mqqapi://&sender=ææºQQ"), 3 == g && (i += "&callback=mqqbrowser://&sender=QQæµè§å¨")), f = 1 == c ? i : b.os.iphone && d.pay && d.cid ? a.app.getPayUrl(d.cid) : j.getDownloadUrl(d.downloadUrl, d.appName), {
                        hasApp: c,
                        pageType: g,
                        os: h,
                        version: e,
        ...
        function(a) {
            var b = function(a, c) {
                return function(d, e) {
                    ...
                    return f.$ = f.$ || c + ".push('" + d.replace(/%%/g, "%%%%").replace(/[%r%t%n]/g, " ").split("<%").join("	").replace(/((^|%>)[^%t]*)'/g, "$1%r").replace(/%t=(.*?)%>/g, "',$1,'").split("	").join("');").split("%>").join(c + ".push('").split("%r").join("%%'") + "');return " + c, e ? f(e) : f
                }
            }({}, "$" + +new Date);
            a.tmpl = b
        }(tvp.$),
                        ...
                        if (b.each(e.config.plugins, function(a, d) {
                            ...
                            for (var f in d)
                                if ("function" == typeof window[f]) {
                                    ...
                                    console.warn("%n" + a.name + "提示：%n您当前页面使用了" + f + "%n建议您引用" + a.name + " for " + f + "专用版，更轻更快更精简%nJS地址:http://imgcache.gtimg.cn/tencentvideo_v1/tvp/js/tvp.player_v2_" + d[f] + ".js%n%n")
                                }
                        }
                    })
                },
