	...
	function loadMeinv() {
		...
		for(var i = 0; i < 10; i++) { //每次加载时模拟随机加载图片
			...
			html = $('<div class="list-goods box boy fl"><div class="goods-index-div"><span class="goods-index">' + 
				...
				data
+'</div>票</div><button class="game-btn-send fr" type="button">送花</button><div class="clear"></div></div></div>').find(".game-btn-send").on('click', function() {
				touPiao(this)
			}).end();
			$minUl = getMinUl();
			$minUl.append(html);
...
//			loadMeinv();//加载新图片
//		}
		console.log($(window).scrollTop(),$(window).height(),$('.game').height())
		if($('.game').height()<= $(window).scrollTop() + $(window).height() + 100) {
			//当最短的ul的高度比窗口滚出去的高度+浏览器高度大时加载新图片
			loadMeinv();//加载新图片
		}
