...
(function(b, d, a) {
	...
	*/
	...
	(b.GAMEUI = function(c, d, e) {
		...
		this.scoretxt.setTransform(484.8, 31);
		this.timeline.addTween(a.Tween.get({}).to({
			state: [{
				t: this.scoretxt
			}, {
	...
	(b.INTROUI = function() {
		...
		//this.shape.graphics.f("#EEEEEEE").p("不显示");
...
function GameOver(b) {
	...
	this.init = function(d) {
		...
		//游戏结束后显现内容
		document.getElementById('game-hdgz').style.display="block";
		var numberType = document.getElementById('number-type').value;
		document.getElementById('game-content').style.display="block";
		//未领奖且积分大于等于4000
		if(GD.SCORE>=4000 && numberType==0)
		{
			document.getElementById('game-lqjp').style.display="block";
		}
		else
		{
			document.getElementById('game-again').style.display="block";
		}
...
function GameMain(b) {
	...
	this.startGame = function() {
		//alert('开始游戏')
		document.getElementById('game-hdgz').style.display="none";
		this.toGameScene()
	};
...
GD.TIMESTRING = "00:00";
GD.PLAYEDNUM = 0;
GD.DESC = "30秒决战邪恶南瓜！一大波小苹果来袭，拳力出击！";
GD.TITLE = "暴击0个邪恶南瓜！";
GD.COMMENT = "手残......";
